<template>
  <div>
    <div class="buttonContGen2">
      <button class="button poke" v-on:click="chickorita">Chickorita</button>
      <button class="button poke" v-on:click="cyndaquil">Cyndaquil</button>
      <button class="button poke" v-on:click="totodile">Totodile</button>
    </div>
    <div class="pokeCont">
      <h2>Generation Two Starters</h2>
      <p>{{pokeDisplay.species.name}}</p>
      <img alt="sprite" v-bind:src="pokeDisplay.sprites.front_default" />
      <p>Type: {{pokeDisplay.types[0].type.name}}</p>
    </div>
  </div>
</template>

<script>
import axios from "axios";
export default {
  data() {
    return {
      numGen2: 152,
      pokeDisplay: ""
    };
  },
  methods: {
    fetchPokemon: function() {
      axios
        .get(`https://pokeapi.co/api/v2/pokemon/${this.numGen2}/`)
        .then(res => {
          this.pokeDisplay = res.data;
        });
    },
    cyndaquil: function() {
      this.numGen2 = 155;
      this.fetchPokemon();
    },
    chickorita: function() {
      this.numGen2 = 152;
      this.fetchPokemon();
    },
    totodile: function() {
      this.numGen2 = 158;
      this.fetchPokemon();
    }
  },
  mounted: function() {
    this.fetchPokemon();
  }
};
</script>

<style scoped>
.buttonContGen2 {
  height: 30%;
  display: flex;
  justify-content: space-evenly;
  align-items: center;
}

.pokeCont {
  height: 50vh;
  background-color: #a58cf9a1;
  display: flex;
  flex-direction: column;
  justify-content: space-between;
  align-items: center;
}
</style>